...
export const mapCampagneFromBackend = (c: any): Campagne => ({
  ...
  dateFin: c.end_date,
  equipeTesteurs: (c.testers || []).map((u: any) => String(u.id)),
  equipeDeveloppeurs: (c.developers || []).map((u: any) => String(u.id)),
  chefTesteurId: c.test_lead_id ? String(c.test_lead_id) : '',
...
export const mapCampagneToBackend = (c: Partial<Campagne>) => ({
  ...
  test_lead_id: c.chefTesteurId ? parseInt(c.chefTesteurId) : undefined,
  testers: c.equipeTesteurs?.map(id => parseInt(id)) || undefined,
  developers: c.equipeDeveloppeurs?.map(id => parseInt(id)) || undefined,
});
...
export const mapNotificationFromBackend = (n: any): Notification => ({
  ...
  titre: n.notification_type === 'anomaly_reported'
    ...
    ? 'Anomalie réouverte'
    : 'Notification',
  message: n.anomaly_description || '',
  lue: !!n.is_read,
  dateCreation: n.created_at,
  lienUrl: n.anomaly_id ? `/anomalies/${n.anomaly_id}` : undefined,
});
